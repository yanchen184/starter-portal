...
- **CryptoUtils** — AES/CBC/PKCS5 加解密工具（Grails 相容，可解密舊系統資料）
...
| 類別 | 方法 | 說明 |
|------|------|------|
| `CryptoUtils` | `aesEncryptBase64(source, key)` | AES 加密 → Base64 |
| `CryptoUtils` | `aesDecryptBase64(source, key)` | Base64 → AES 解密 |
| `IpUtils` | `getClientIp(request)` | 從 X-Forwarded-For 取得真實 Client IP（含格式驗證防 log spoofing） |

> **注意：** `CryptoUtils` 的 IV 由 key 衍生（MD5），為了與 Grails 舊系統相容。新專案如果不需要解密舊資料，建議使用 `SecureRandom` 產生隨機 IV。
